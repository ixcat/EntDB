...
#
GROUP       = $(ENTDBDIR)/group
PASSWD      = $(ENTDBDIR)/passwd
SHADOW	    = $(ENTDBDIR)/shadow
GSHADOW     = $(YPPWDDIR)/gshadow
ADJUNCT     = $(ENTDBDIR)/passwd.adjunct
BSDPASSWD   = $(ENTDBDIR)/master.passwd
...
all:  passwd group hosts rpc services netid protocols mail \
	passwd.adjunct master.passwd shadow netgroup
...
master.passwd.byname: $(BSDPASSWD) $(YPDIR)/Makefile
	@echo "Updating $@..."
	@$(UMASK); \
	$(AWK) -F: '!/^[-+#]/ { if ($$1 != "" && $$3 >= $(MINUID) && $$3 != $(NFSNOBODYUID) ) \
	   print $$1"\t"$$0 }' $(BSDPASSWD) | $(DBLOAD) -s -i $(BSDPASSWD) \
		-o $(YPMAPDIR)/$@ - $@
	-@$(NOPUSH) || $(YPPUSH) -d $(DOMAIN) $@

master.passwd.byuid: $(BSDPASSWD) $(YPDIR)/Makefile
	@echo "Updating $@..."
	@$(UMASK); \
	$(AWK) -F: '!/^[-+#]/ { if ($$1 != "" && $$3 >= $(MINUID) && $$3 != $(NFSNOBODYUID) ) \
	   print $$3"\t"$$0 }' $(BSDPASSWD) | $(DBLOAD) -s -i $(BSDPASSWD) \
		 -o $(YPMAPDIR)/$@ - $@
	-@$(NOPUSH) || $(YPPUSH) -d $(DOMAIN) $@
