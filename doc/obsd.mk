...
passwd.time: $(ENTDIR)/master.passwd
	-@if [ -f $(>) ]; then \
		if [ ! $(UNSECURE) ]; then \
			$(CAT) $(>) | $(CUT) -d: -f1-4,8-10 | \
		...
		{ print $$1, $$0 }' - | $(MAKEDBM-S) - shadow.byname; \
		$(CAT) $(ENTDIR)/passwd.adjunct | \
		$(AWK) 'BEGIN { FS=":"; OFS="\t"; } /^[a-zA-Z0-9_]/ \
		{ print $$1, $$0 }' - | $(MAKEDBM-S) - passwd.adjunct.byname; \
		$(TOUCH) $(@); \
		$(ECHO) "updated passwd"; \
		if [ ! $(NOPUSH) ]; then \
...
netid.time: $(ENTDIR)/passwd $(ENTDIR)/master.passwd $(ENTDIR)/group $(DIR)/hosts $(DIR)/netid
	-@$(MKNETID) -q -d $(DOMAIN) -p $(ENTDIR)/passwd -P $(ENTDIR)/master.passwd \
		-g $(ENTDIR)/group -h $(DIR)/hosts -m $(DIR)/netid | \
